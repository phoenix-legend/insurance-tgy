class UserSystem::CarBusinessUserInfo < ActiveRecord::Base
  BusinessWords = ["诚信", '到店', '精品', '车行', '第一车网', '车业', '信息编号', '本公司','本店', '提档', '双保险', '可按揭', '该车为', '铲车', '首付', '全顺', '该车', '按揭', '热线', '瓜子']
  ...
  def self.add_business_user_info_phone car_user_info
    ...
    if not car_user_info.name.blank?
      if is_cheshang == false
        ['经理', '总', '老板','图','号码','归属','商家', '赶集', '瓜子','名车', '二手车', '黄牛', '销售', '顾问', '阳光车网', '客服', '车王', '看图',
         '看内容', '最多填写6字', '优车', '车置宝', '天天', '标题', '大爷'].each do |name_key|
  ...
  def self.is_pachong car_user_info
    ...
    if not car_user_info.name.blank?
      ...
      ['商家', '赶集', '瓜子', '销售', '套牌','百姓','车行','名车', '顾问', '阳光车网', 'QQ','qq','微信','Qq','图片','客服', '管家', '车王', '看图', '58', '五八', '之家', '内容', '最多填写6字', '优车', '车置宝', '天天', '标题', '大爷'].each do |name_key|
        if car_user_info.name.include? name_key
          return true
        end
      end
